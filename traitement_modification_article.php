...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES['nouvelle_image']['name'])) {
        ...
        if (move_uploaded_file($file_tmp, $upload_path)) {
            ...
            $query = "UPDATE articles SET titre = :nouveau_titre, contenu = :nouveau_contenu, media_url = :media_url, media_type = :media_type WHERE id = :article_id";
            $stmt = $connexion->prepare($query);
        ...
        $query = "UPDATE articles SET titre = :nouveau_titre, contenu = :nouveau_contenu WHERE id = :article_id";
        $stmt = $connexion->prepare($query);
        $stmt->bindParam(':nouveau_titre', $nouveau_titre, PDO::PARAM_STR);
        $stmt->bindParam(':nouveau_contenu', $nouveau_contenu, PDO::PARAM_STR);
        $stmt->bindParam(':article_id', $article_id, PDO::PARAM_INT);
