...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES['nouvelle_image']['name'])) {
        ...
        if (move_uploaded_file($file_tmp, $upload_path)) {
            ...
            $query = "UPDATE articles_de_presse SET titre = :nouveau_titre, contenu = :nouveau_contenu, media_url = :media_url, media_type = :media_type, langue = :nouvelle_langue WHERE id = :article_id";
            $stmt = $connexion->prepare($query);
            ...
            $stmt->bindParam(':nouvelle_langue', $nouvelle_langue, PDO::PARAM_STR);
            $stmt->bindParam(':article_presse_id', $article_presse_id, PDO::PARAM_INT);
        ...
        // Si aucun nouveau fichier image a été téléchargé, mettez à jour uniquement le titre et le contenu
        $query = "UPDATE articles_de_presse SET titre = :nouveau_titre, contenu = :nouveau_contenu, langue = :nouvelle_langue WHERE id = :article_presse_id";
        $stmt = $connexion->prepare($query);
        $stmt->bindParam(':nouveau_titre', $nouveau_titre, PDO::PARAM_STR);
        $stmt->bindParam(':nouveau_contenu', $nouveau_contenu, PDO::PARAM_STR);
        $stmt->bindParam(':article_presse_id', $article_presse_id, PDO::PARAM_INT);
